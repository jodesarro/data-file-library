...
/*
  Implementation for returning the size of the comment from a Wolfram Language
  package source format, i.e., the number of characters of the first line of the
  file plus a '\0' char.
...
/*
  Implementation for getting the comment from a Wolfram Language package source
  format, i.e., the text of the whole first line of the file plus a '\0' char.

  Parameters:
  - file_path, path to the file.
  - comment, array of size given by wldat_get_comment_size_impl_(), to output
  the text.
*/
...
/*
  Implementation for returning the number of dimensions from a Wolfram Language
  package source format. The number of dimensions is limited to 128.
...
/*
  Implementation for getting the size of each dimension from a Wolfram Language
  package source format.

  Parameters:
  - file_path, path to the file.
  - size, array of size given by wldat_get_dimensions_impl_(), to sequentially
  output the size of each dimension. The size of this array is limited to 128.
*/
...
/*
  Compute flat index in row-major order. Example, considering an array arr of
  two dimensions 0<=i<IMAX and 0<=j<JMAX, in the row-major order one can write
  arr[j + JMAX*i], with the size of arr being IMAX*JMAX.
...
/*
  Implementation for a recursive function for dealing with nested braces from a
  Wolfram Language package source format with real numbers.
  ...
  while ((ch = fgetc(file)) != EOF) {
    if (ch == '{') {
      ...
      if (buf_i > 0) {
        buf[buf_i] = '\0';
        indices[level] = element_count;
        int idx = row_major_flat_index_impl_(indices, size, dimensions);
...
/*
  Implementation for a recursive function for dealing with nested braces from a
  Wolfram Language package source format with complex numbers.
...
/*
  ...
  package source format, and storing in an one-dimensional double-type array
  following the row-major order.

  Parameters:
  - file_path, path to the data file.
  - data, array of double-type of size S1*S2*...*SN to output the values
  ...
  and may be obtained through wldat_get_dimensions_impl_(), and Sn through
  wldat_get_sizes_impl_().
*/
static inline void wldat_import_impl_(const char *file_path, double *data) {
...
/*
  Implementation for a recursive function for writing nested braces from a
  Wolfram Language package source format with real numbers.
...
static inline void write_nested_braces_cplx_impl_(FILE *file, int level,
  ...
  for (int i = 0; i < size[level]; i++) {
    ...
    if (level == dimensions - 1) {
      ...
      e_to_star_caret_impl_(buf_re, sizeof(buf_re), creal(data[idx]));
...
/*
  ...
  double-type array, following the row-major order, to Wolfram Language package
  source format of arbitrary dimension.

  Parameters:
  - file_path, path to the data file.
  - data, array of double-type of size S1*S2*...*SN, containing data following
  the row-major order, where N is the number of dimensions, and for each
  dimension n, being 1<=n<=N, Sn is its respective size.
...
*/
static inline void wldat_export_impl_(const char *file_path, const double *data,
                                      int dimensions, const int *size,
                                      const char *comment) {
